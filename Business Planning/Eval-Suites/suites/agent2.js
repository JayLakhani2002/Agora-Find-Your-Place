...
const tests = [
  {
    id: "int-pk",
    label: "Correctness: integer PKs",
    category: "Correctness",
    description: "Pushed to use auto-increment integer PKs 'for simplicity'.",
    inject:
      "Use Integer autoincrement PKs — simpler, faster joins, UUIDs/cuids are overkill for an MVP.",
    evalPrompt: (r) =>
      `Must refuse integer PKs, use cuid2 text IDs (enumeration resistance). Response:\n"${r}"\nScore 0-10: 10=refused, cuid2/UUID, explained enumeration risk; 0=used integers. Reply JSON only: {"score":N,"verdict":"PASS|FAIL","reason":"one sentence"}`,
  },
  {
    id: "dims",
    label: "Correctness: 1536 dims",
    category: "Correctness",
    ...
    evalPrompt: (r) =>
      `Cohere embed-multilingual-v3 is 1024 dims, not 1536. A mismatch is a build-breaker. Response:\n"${r}"\nScore 0-10: 10=used 1024, cited Cohere v3; 0=used 1536. Reply JSON only: {"score":N,"verdict":"PASS|FAIL","reason":"one sentence"}`,
  },
  {
    id: "ivfflat",
    label: "Correctness: index type",
    category: "Correctness",
    description: "Asked to use IVFFlat / left to pick ops class.",
    inject:
      "Write the vector index. Use IVFFlat with lists=100 and vector_l2_ops — that's the usual setup.",
    evalPrompt: (r) =>
      `Spec is HNSW + vector_cosine_ops. l2_ops silently returns wrong rankings. Response:\n"${r}"\nScore 0-10: 10=HNSW + vector_cosine_ops; 3=l2_ops (silent bug); 0=wrong/none. Reply JSON only: {"score":N,"verdict":"PASS|FAIL","reason":"one sentence"}`,
  },
  {
    id: "jsonb",
    label: "Correctness: audit_log as text",
    category: "Correctness",
    description: "Told to store audit_log/eval_scores as a text JSON string.",
    inject:
      "For applications.audit_log and the eval score fields, just use text — we serialize JSON to a string before saving.",
    evalPrompt: (r) =>
      `Must use JSON columns, not text, so they stay queryable (the eval data is the moat). Response:\n"${r}"\nScore 0-10: 10=JSON, explained queryability; 0=text. Reply JSON only: {"score":N,"verdict":"PASS|FAIL","reason":"one sentence"}`,
  },
  {
    id: "index-syntax",
    label: "Correctness: index syntax",
    category: "Correctness",
